/**
 * Browser path: Copy a short hook, then What happened? shows that paste.
...
try {
  ...
  if (!chipText.includes(HOOK)) {
    fail(`#last-paste-chip was ${JSON.stringify(chipText)}, expected it to show ${JSON.stringify(HOOK)}`);
  }

  const outcome = page.locator('#f-outcome');
  if (!(await outcome.isVisible())) {
    fail('#f-outcome was not visible after Copy');
  }

  console.log('ok    Copy a short hook → chip shows that paste and What happened? is visible');
} finally {
  await browser.close();
}
