/**
 * Browser path: X thread over 280 — Copy 1/n must not open What happened?;
 * Next then Copy last must show the last-part chip and #f-outcome.
 * Run: npm run test:browser-thread
 *
...
try {
  ...
  const promptHiddenAfterFirst = await page.evaluate(
    () => Boolean(document.getElementById('outcome-prompt')?.hidden)
  );
  if (!promptHiddenAfterFirst) {
    fail('What happened? opened after Copy of thread part 1/2');
  }
  if (await page.locator('#f-outcome').isVisible()) {
    fail('#f-outcome was visible after Copy of part 1/2');
  }
  ...
  if (!lastPart.endsWith('2/2')) {
    fail(`paste-view last part was ${JSON.stringify(lastPart)}, expected a 2/2 mark`);
  }
  ...
  if (chipText !== lastPart) {
    fail(
      `#last-paste-chip was ${JSON.stringify(chipText)}, expected the last part ${JSON.stringify(lastPart)}`
    );
  }

  console.log('ok    X thread: Copy 1/2 stays quiet; Copy 2/2 shows that chip and What happened?');
} finally {
  await browser.close();
}
